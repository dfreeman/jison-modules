...
module.exports = function rewriteModule(module, exportMap) {
  ...
  namespaceLexicon(prefix, importMapping, module.lexicon);
...
function namespaceLexicon(prefix, mapping, lexicon) {
  if (!lexicon) return;

  var macroRegex = /\{([-_a-zA-Z]+)\}/g;
  var macroReplacement = function(match, identifier) {
    return '{' + prefixIdentifier(identifier, prefix, mapping).replace(/\$/g, '$$') + '}';
  };
  ...
  lexicon.rules.forEach(function(rule) {
    ...
    if (Array.isArray(rule[0])) {
      index = 1;
      rule[0] = rule[0].map(function(startCondition) {
        return startCondition === 'INITIAL' ? startCondition : prefixIdentifier(startCondition, prefix, mapping);
      });
    }
