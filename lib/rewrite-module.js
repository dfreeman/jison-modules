...
module.exports = function rewriteModule(module, exportMap) {
  ...
  namespaceOperators(prefix, module.operators);
...
function namespaceOperators(prefix, operators) {
  operators.forEach(function(operator) {
    for (var i = 0, len = operator.tokens.length; i < len; i++) {
      operator.tokens[i] = prefix + operator.tokens[i];
    }
  });
}
...
function buildImportMapping(imports, exportMapping) {
  var mapping = {};
  imports.forEach(function(imp) {
    mapping[imp.binding] = prefixify(imp.module) + exportMapping[imp.module][imp.source];
  });
  return mapping;
}
