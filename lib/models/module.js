...
function Module(name, options) {
  ...
  if (options.grammar && options.grammar.operators) {
  ...
  var grammarDecls = extractDecls(options.grammar && options.grammar.unknownDecls, ['import', 'export', 'precedence']);
  var lexiconDecls = extractDecls(options.lexicon && options.lexicon.unknownDecls, ['import', 'export', 'priority']);
  ...
  this.addImports(extractImportDeclarations(grammarDecls.import, false));
  this.addImports(extractImportDeclarations(lexiconDecls.import, true));
  this.addExports(extractExportDeclarations(grammarDecls.export, false));
  this.addExports(extractExportDeclarations(lexiconDecls.export, true));
...
function extractImportDeclarations(decls, lexical) {
...
function extractExportDeclarations(decls, lexical) {
  return flatten(decls.map(function(decl) {
    var specifier = ModuleDeclParser.parse(decl);
    return specifier.bindings.map(function(binding) {
      return {
